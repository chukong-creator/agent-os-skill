# Agent OS v0.3 five-question maturity contract

## The five required answers

Every v0.3 Run must answer five questions from inspectable artifacts bound to one `run_id` and accepted `branch_commit`.
...
Automatic rollback is intentionally narrow. It requires a `MERGED` Run, recorded merge commit, clean protected base, `HEAD == merge_commit`, exactly two merge parents, second parent equal to the accepted branch commit, consistent Agent Shift state, no active writer, passing accepted evidence, explicit `--execute`, and a reason.
...
Use `Observation -> Hypothesis -> Proposal -> Experiment -> Adopt or Reject`.
...
Before `ACCEPTED`, require: Evidence Manifest PASS, Verifier PASS, exact commit match, no open failure, Learning Assessment, Maturity Report PASS, Review, and a Merge Gate rerun after any director-owned Review commit changes the protected branch.
